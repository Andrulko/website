...
{{% def id="IdenTrust" name="IdenTrust" %}} A [Certificate Authority](#def-CA). IdenTrust has [cross-signed](#def-cross-signing) [Let's Encrypt](#def-LE) [intermediate certificates](#def-intermediate): <https://letsencrypt.org/certificates/>. [Wikipedia](https://en.wikipedia.org/wiki/IdenTrust) {{% /def %}}
